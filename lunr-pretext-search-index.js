...
},
{
  ...
  "number": "11",
  "title": "Macaulay 2",
  "body": " Macaulay 2   This chapter is co-authored by Francesca Gandini, Sumner Strom, Al Ashir Intisar (push test successfull)    Template  This is the introduction to the template.   Template Subsection  This is the template subsection.    Notes Subsection  To build in pretext use the template subsection and pretext environment features. Make sure that you are building and compiling from main using preview code chat. Additionally, when running if the compilation fails run the command \"pretext build web\" in terminal. Make sure that all of the xml have unique ids so that you dont run into compilation errors. Follow the other chapters to find out how things are used. Make sure that the sections are properly linked within the main.ptx. Additionally, use the subsections and sections for organization. If something doesnt display there is a paragraph line missing.     Creating a M2 Codespace  A turn-key repository for creating a Codespace ( ) for Macaulay 2 is available at fragandi\/M2-codespace . Below we provide detailed instructions. If you have some experience with codespaces, you might be able to follow directly the README in the repo. Otherwise, keep reading!  (WRITE DOWN NOTES HOW TO START A CODESPACE AND USE THE M2 CODESPACE IN THE ABOVE PARAGRAPH.)    Basic M2 commands    InvariantRings package    Theory for invariant rings  hello this is true.  Nöether: NDB: The ring of invariants is generated in degrees      "
},
{
